...
async def add_item(
    ...
    required_fields = ["itemId", "itemName", "preferredZone", "priority", "width", "height", "depth"]
    for field in required_fields:
        if field not in item:
            raise HTTPException(status_code=400, detail=f"Missing required field: {field}")
    
    # Add default usageCount if not provided
    if "usageCount" not in item:
        item["usageCount"] = 0
    
    # Check if item already exists
    if not cargo_system.items_df.is_empty():
        existing_item = cargo_system.items_df.filter(pl.col("itemId") == item["itemId"])
        if not existing_item.is_empty():
            raise HTTPException(status_code=400, detail="Item with this ID already exists")
    
    # Add new item to DataFrame
    new_item_df = pl.DataFrame([item])
    if cargo_system.items_df.is_empty():
        cargo_system.items_df = new_item_df
    else:
        cargo_system.items_df = cargo_system.items_df.vstack(new_item_df)
...
async def add_container(
    ...
    # Ensure required fields exist
    required_fields = ["zone", "width", "height", "depth"]
    for field in required_fields:
        if field not in container:
            raise HTTPException(status_code=400, detail=f"Missing required field: {field}")
    
    # Check if container with same zone already exists
    if not cargo_system.containers_df.is_empty():
        existing_container = cargo_system.containers_df.filter(pl.col("zone") == container["zone"])
        if not existing_container.is_empty():
            raise HTTPException(status_code=400, detail="Container with this zone already exists")
    
    # Add new container to DataFrame
    new_container_df = pl.DataFrame([container])
    if cargo_system.containers_df.is_empty():
        cargo_system.containers_df = new_container_df
    else:
        cargo_system.containers_df = cargo_system.containers_df.vstack(new_container_df)
    
    # Add to octrees
    try:
        zone = container["zone"].strip()
        cargo_system.octrees[zone] = cargo_system.Octree(container)  # Assuming Octree class is accessible
    except Exception as e:
        cargo_system.loading_log.append(f"Error creating octree for zone {container['zone']}: {str(e)}")
    
    # Save to CSV
    cargo_system.containers_df.write_csv(CONTAINERS_CSV_PATH)
